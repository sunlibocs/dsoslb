...
//residuals 数组大小为framehessian中frame的数目
//minobs为1
...
{
	int nres = 0;
	//对point 看frameHessians中的每一个frame是否为他的host frame
	for(FrameHessian* fh : frameHessians)
	{
		//如果frame不是要计算的point的host frame则不存在存储残差（残差来源于host中）
		if(fh != point->host)
		{
	...
	//线性残差把每一个nres的结果加一块, 如lastEnergy, 'H b 这里也加了，为啥'  
