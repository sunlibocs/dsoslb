...
//计算残差，传入参数为层号lvl，RF到当前帧的转换

//计算一个点在ref中的灰度差 - 在经过变换后在当前帧中的位置的灰度差，并且所有求和
...
//所有点的总残差
	...
	//参与计算残差的点的数目
	rs[1] = numTermsInE;
	//第0层，点在ref中的坐标 - 点在当前帧中的坐标（经过在ref中反投影并且旋转得到） 的均值
	rs[2] = sumSquaredShiftT/(sumSquaredShiftNum+0.1);
	rs[3] = 0;
	//和上面的不同之处在于，上面只用了平移，但是这个用到了旋转和平移
...
Vec6 CoarseTracker::calcRes(int lvl, const SE3 &refToNew, AffLight aff_g2l, float cutoffTH)
{
	...
	for(int i=0;i<nl;i++)
	{
		...
		float x = lpc_u[i];
		float y = lpc_v[i];
		//转换到当前帧，后面point都是当前帧中的坐标，但是x,y是ref中的坐标
		//得到当前点的在当前层的三维坐标
		//计算公式为先反投影到空间中，之后RX + t * 当前层的因子
		...
		//参考帧的原始图像（第0层）中的像素坐标位置 - 该点在当前帧中的坐标位置
		if(lvl==0 && i%32==0)
		{
			...
			//already have it.
			//ref中的坐标值 - 当前帧中的坐标值（通过转换求得的） 平方
			//两个之间必然会存在位置的差异，不可能在同一个位置上，除非没动。
		...
		IOWrap::Output3DWrapper* wrap)
{
	...
	for(int lvl=coarsestLvl; lvl>=0; lvl--)
	{
		...
		//得到H和b，这里track的时候只优化相两帧的相对状态（相对位姿 6 + 光度仿射变换 2）
...
//这里值越大表示越不可能落在这个位置上！！！
//×该函数用已经投影后的actived 的位置的值去更新它周围点的值×
//×××××important 一个位置值越小表明该点附近越有actived的点的投影的存在

//如果当前点不在，则值考虑当前点的上下左右位置是否有距离在0-40内的点，如果有则设置为那个0-40的最小的值
...
//fwdWarpedIDDistFinal 中添加当前的距离为0
