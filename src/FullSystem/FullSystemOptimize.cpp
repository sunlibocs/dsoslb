...
void FullSystem::linearizeAll_Reductor(bool fixLinearization, std::vector<PointFrameResidual*>* toRemove, int min, int max, Vec10* stats, int tid)
{
	...
	for(int k=min;k<max;k++)
	{
		...
		if(fixLinearization)
		{
			...
			//如果为false只push进activeResiduals元素
			else
			{
...
//设置它们的 PointFrameResidual::ResState
//（按照 FullSystem::optimizeImmaturePoint 的结果），
//如果是正常的点(ResState::IN)调用EFResidual::takeDataF()把 EFResidual::JpJdF 设置一下。
//这就算完成了优化的准备工作。
...
//fixLinearization 为真的时候才会在linearizeAll_Reductor中applyRes设置JpJdF设置JpJdF
//false则添加到toRemove
...
float FullSystem::optimize(int mnumOptIts)
{
	...
	//这里为false
	//计算导数参数 J
	//push activeResiduals中的每个元素到toRemove
	...
	for(int iteration=0;iteration<mnumOptIts;iteration++)
	{
		...
		//solveSystemNew(0);
		//FullSystem::solveSystem() 进行优化（得到的优化变化量 step）
		...
		//FullSystem::doStepFromBackup() 将优化结果生效
		//返回是否可以结束的标志
		...
		//迭代可以，或者能量比之前的小了
		//applyRes_Reductor 调用takeDataF计算ＥＦＪ
	...
	//效果是将优化之后成为 outlier 的 residual 剔除，剩下正常的 residual 
	//调用一次 EFResidual::takeDataF() 计算新的 EFResidual::JpJdF （FEJ）
