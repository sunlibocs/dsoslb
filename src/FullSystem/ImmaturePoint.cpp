...
: u(u_), v(v_), host(host_), my_type(type), idepth_min(0), idepth_max(NAN), lastTraceStatus(IPS_UNINITIALIZED)
{
	...
	for(int idx=0;idx<patternNum;idx++)
	{
		...
		//返回一个vector
		//内容依次为（a左边梯度 + b右边梯度，右边梯度 - 左边梯度， c下边梯度 -b上面梯度）
...
//	const float minImprovementFactor = 2;		// if pixel-interval is smaller than this, leave it be.
	...
	//搞了一堆就是为了求解出point在当前frame 中的预测坐标
	...
	for(int i=0;i<numSteps;i++)
	{
		...
		for(int idx=0;idx<patternNum;idx++)
		{
			...
			//计算残差
			//这里color应该是host中的点，也就是本immpoint的实例
	...
	//newquality must be bigger than 1
	// 
...
//传入参数为相机内参，1，存储残差，H，b,depth
//投影，如果有有一个不行，后面的pattern以及在别的frame中的投影都不用算了
//点直接设置为了OOB
		...
		float idepth)
{
	...
	//tmpRes->target->idx 得到一个frame的id
	FrameFramePrecalc* precalc = &(host->targetPrecalc[tmpRes->target->idx]);
